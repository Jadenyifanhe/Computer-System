 ...
 * @author Yuchenwu <yuchenwu@andrew.cmu.edu>
 */
...
static void trans_1(size_t M, size_t N, const double A[N][M], double B[M][N]) {
    int j, k, kk, jj;
    for (kk = 0; kk < 32; kk += 8) {
        for (jj = 0; jj < 32; jj += 8) {
            for (j = 0; j < 8; j++) {
                for (k = j; k < 8; k++)
                    B[j + jj][k + kk] = A[kk + k][jj + j];
                for (k = j - 1; k > -1; k--)
                    B[j + jj][k + kk] = A[kk + k][jj + j];
            }
        }
...
static void trans_2(size_t M, size_t N, const double A[N][M], double B[M][N]) {
    int j, k, kk, jj;
    for (kk = 0; kk < 1024; kk += 8) {
